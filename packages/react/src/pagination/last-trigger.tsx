...
const paginationLastTrigger = cva(
...
export interface PaginationLastTriggerProps extends React.ComponentProps<typeof Ark.LastTrigger> {}

export function PaginationLastTrigger({
  className,
  children,
  ...rest
}: PaginationLastTriggerProps) {
  const variants = usePaginationVariants();
  const inner = <>{children ?? <ChevronsRight />}</>;
   ...
   */
  return (
    <Ark.LastTrigger {...rest} asChild>
      {variants.linked ? (
        <a
          data-slot="pagination-last-trigger"
          className={cn(paginationLastTrigger(variants), className)}
        >
          {inner}
        </a>
      ) : (
