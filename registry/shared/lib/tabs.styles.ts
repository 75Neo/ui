...
export const tabs = tv({
  slots: {
    ...
    trigger:
      "inline-flex cursor-pointer items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium text-muted transition-colors hover:text-default focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary data-disabled:pointer-events-none data-disabled:opacity-75 data-selected:text-default",
    indicator:
      "absolute -bottom-px left-(--left) h-0.5 w-(--width) rounded-full bg-primary transition-[left,top,width,height] duration-200 ease-out data-[orientation=vertical]:-end-px data-[orientation=vertical]:top-(--top) data-[orientation=vertical]:bottom-auto data-[orientation=vertical]:left-auto data-[orientation=vertical]:h-(--height) data-[orientation=vertical]:w-0.5",
