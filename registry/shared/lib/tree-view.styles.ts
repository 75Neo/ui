...
const node =
  "flex cursor-pointer items-center gap-1.5 rounded-md py-1.5 pe-2 text-sm text-muted transition-colors select-none ps-[calc(0.75rem*var(--depth))] hover:bg-muted hover:text-default focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-inverted data-disabled:pointer-events-none data-disabled:opacity-75 data-selected:bg-muted data-selected:text-default";
...
export const treeView = tv({
  slots: {
    ...
    nodeRenameInput:
      "w-full min-w-0 rounded-sm bg-default px-1 text-sm text-default ring ring-inverted outline-none",
  },
});
