...
export const progress = tv({
  slots: {
    ...
    range: "h-full rounded-full transition-[width] duration-300 ease-out",
    valueText: "text-sm text-muted tabular-nums",
  },
  variants: {
    color: {
      primary: { range: "bg-primary" },
      secondary: { range: "bg-secondary" },
      success: { range: "bg-success" },
      info: { range: "bg-info" },
      warning: { range: "bg-warning" },
      error: { range: "bg-error" },
